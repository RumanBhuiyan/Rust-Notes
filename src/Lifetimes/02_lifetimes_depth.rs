...
/*
     In below example after returning reference of &country , country will be dropped
     so c will be dangling pointer but Rust doesn't support it.
     So to quick fix it, one solution can be returning ownership instead of returning reference.
 */
